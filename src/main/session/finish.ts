...
import { draftFastFollowup, conversationMessages, automaticFinishEnabled, goalDrivingMode, goalObjectiveFor, goalProgressFor, loopTimerMsFor, onGoalChange } from '../goal.js';
...
async function prepareNotice(sessionId: string, summary: string, userRequested = false, requestedTurnId?: string): Promise<string> {
  ...
  const work = (async () => {
    ...
    // A configured Loop timer is exempt: its cadence already says when the next step is due,
    // so neither the decision nor its pickup waits for the workers.
    if (session.conversationId && waitingForSubAgents(session.conversationId) && loopTimerMsFor(session.conversationId) === 0) {
